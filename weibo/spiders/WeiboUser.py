...
class WeiboUserSpider(RedisSpider):
    name = 'WeiboUser'
    ...
    def parse_user(self, response):
        ...
        for i in range(count):
            ...
            weiboItem['verified'] = json_object['data']['cards'][i]['user']['verified']
            #weiboItem['verified_reason'] = json_object['data']['cards'][i]['user']['verified_reason']
            yield Request(self.start_profile_url.format(weiboid=weiboItem['id'], include=self.start_include),
                              callback=self.parse_homepage,dont_filter=True)
    ...
    def parse_followers(self, response):
        ...
        weiboItem = InformationItem()
        try:
           last_id=json_object['data']['cardlistInfo']['containerid'].split('_-_')[-1]
        except Exception as e:
            log.debug('no cardlistInfo')
        if json_object['data']['cards']:
            count = -1
            j=len(json_object['data']['cards'])-1
            for i in json_object['data']['cards'][j]['card_group']:
                count += 1
            for i in range(count):
                weiboItem['screen_name'] = json_object['data']['cards'][j]['card_group'][i]['user']['screen_name']
                weiboItem['description'] = json_object['data']['cards'][j]['card_group'][i]['user']['description']
                weiboItem['follow_count'] = json_object['data']['cards'][j]['card_group'][i]['user']['follow_count']
                weiboItem['followers_count'] = json_object['data']['cards'][j]['card_group'][i]['user']['followers_count']
                weiboItem['gender'] = json_object['data']['cards'][j]['card_group'][i]['user']['gender']
                weiboItem['id'] = json_object['data']['cards'][j]['card_group'][i]['user']['id']
                weiboItem['profile_url'] = json_object['data']['cards'][j]['card_group'][i]['user']['profile_url']
                weiboItem['close_blue_v'] = json_object['data']['cards'][j]['card_group'][i]['user']['close_blue_v']
                weiboItem['desc1'] = json_object['data']['cards'][j]['card_group'][i]['user']['desc1']
                weiboItem['desc2'] = json_object['data']['cards'][j]['card_group'][i]['user']['desc2']
                weiboItem['spider_time'] = now.strftime('%Y-%m-%d %H:%M:%S')
                weiboItem['cover_image_phone'] = json_object['data']['cards'][j]['card_group'][i]['user']['cover_image_phone']
                weiboItem['verified'] = json_object['data']['cards'][j]['card_group'][i]['user']['verified']
                #weiboItem['verified_reason'] = json_object['data']['cards'][j]['card_group'][i]['user']['verified_reason']
        ...
        try:
            if json_object['data']['cards']:
                ...
                for i in range(count):
                    ...
                    weiboItem['description'] = json_object['data']['cards'][j]['card_group'][i]['user']['description']
                    weiboItem['follow_count'] = json_object['data']['cards'][j]['card_group'][i]['user']['follow_count']
                    weiboItem['followers_count'] = json_object['data']['cards'][j]['card_group'][i]['user'][
                        'followers_count']
                    weiboItem['gender'] = json_object['data']['cards'][j]['card_group'][i]['user']['gender']
                    weiboItem['id'] = json_object['data']['cards'][j]['card_group'][i]['user']['id']
                    weiboItem['profile_url'] = json_object['data']['cards'][j]['card_group'][i]['user']['profile_url']
                    ...
                    weiboItem['desc1'] = json_object['data']['cards'][j]['card_group'][i]['user']['desc1']
                    weiboItem['desc2'] = json_object['data']['cards'][j]['card_group'][i]['user']['desc2']
                    weiboItem['cover_image_phone'] = json_object['data']['cards'][j]['card_group'][i]['user'][
                        'cover_image_phone']
                    weiboItem['spider_time'] = now.strftime('%Y-%m-%d %H:%M:%S')
                    weiboItem['verified'] = json_object['data']['cards'][j]['card_group'][i]['user']['verified']
                        #log.info(weiboItem['screen_name'])
                    #weiboItem['verified_reason'] = json_object['data']['cards'][j]['card_group'][i]['user'][
                                #'verified_reason']
                    yield Request(self.fans_profile_url.format(weiboid=weiboItem['id'], last_id=last_id),
                                      callback=self.parse_homepage_for_followers_and_fans, dont_filter=True)
                    yield Request(self.followers_profile_url.format(weiboid=weiboItem['id'], last_id=last_id),
                                      callback=self.parse_homepage_for_followers_and_fans, dont_filter=True)
                    yield weiboItem
        ...
        if json_homepage['data']['cards']:
            ...
            for i in range(count):
                if json_homepage['data']['cards'][i]['card_type'] == 9:
                    ...
                    tweetsitem['ve']=json_homepage['data']['cards'][i]['mblog']['user']['verified']
